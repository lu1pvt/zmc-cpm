...
int main(int argc, char** argv) {
    ...
    if ( bdos( 12, NULL ) == 0x31 ) { // CP/M Plus
    ...
    while( loop ) {
        ...
        if (k == 0x09) { // 1. TAB: Cambio de panel OTHER_PANEL
            other_panel();
        } else if ( ( k >= 'a' && k <= 'p' ) || ( k >= 'A' && k <= 'P' ) ) {
            change_drive( k ); // 2. A-P: Cambio de Unidad CHANGE_DRIVE
        } else if (k == ' ') { // 1. Selección con Espacio (Cierre de ciclo de refresco) SELECT
            select();
        } else if ( k == 'E'-'@') { // ^E
            line_up();
        } else if ( k == 'X'-'@' ) { // ^X
            ...
            k = wait_key_hw();
            if (k == ESC && wait_key_hw() == ESC) { // <ESC><ESC><ESC>
                break;
            } else if (k == '[') { // "<ESC>["
                k = wait_key_hw();
                switch(k) {
		    case 'A': // ""<ESC>[A" LINE_UP
                        line_up();
		        break;
		    case 'B': // "<ESC>[B" LINE_DOWN
                        line_down();
		        break;
		    case '5': // "<ESC>[5~" PAGE_UP
                        wait_key_hw(); // consume '~'
                        page_up();
                        break;
		    case '6': // "<ESC>[6~" PAGE_DOWN
                        wait_key_hw(); // consume '~'
                        page_down();
                        break;
                    case '1': // F5 = "<ESC>[15~" / F8 = "<ESC>[19~"
                        k = wait_key_hw();
                        if (k == '5') { // F5 = "<ESC>[15~" COPY
                            wait_key_hw(); // consume '~'
                            copy();
                        } else if (k == '9') { // F8 = "<ESC>[19~" DELETE
                            wait_key_hw(); // consume '~'
                            delete();
                        }
                        refresh_ui(1);
